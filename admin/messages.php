...
try {
    $stmt = $pdo->prepare("
        ...
        WHERE (m.expediteur_id = ? AND u.role IN ('admin', 'conseiller'))
           OR (m.destinataire_id = ? AND u2.role IN ('admin', 'conseiller'))
...
<style>
/* Mêmes styles compacts que conseiller/messages.php */
* { margin: 0; padding: 0; box-sizing: border-box; }
body { background: #ffffff; font-family: 'Inter', sans-serif; }
.messages-container { max-width: 1000px; margin: 0 auto; padding: 15px; }
.page-header { margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; }
.page-header h1 { font-size: 1.4rem; color: #1f2937; }
.page-header p { font-size: 0.8rem; color: #6b7280; margin-top: 3px; }
.btn-new-message { background: linear-gradient(135deg, #059669, #10b981); color: white; padding: 0.4rem 1rem; border: none; border-radius: 0.5rem; cursor: pointer; display: inline-flex; align-items: center; gap: 5px; font-weight: 600; font-size: 0.85rem; }
.btn-new-message:hover { transform: translateY(-1px); box-shadow: 0 2px 8px rgba(5,150,105,0.3); }
.stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px,1fr)); gap: 12px; margin-bottom: 20px; }
.stat-card { background: #fff; border-radius: 0.75rem; padding: 0.8rem; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05); border: 1px solid #e5e7eb; text-align: center; }
.stat-number { font-size: 1.4rem; font-weight: 700; color: #059669; }
.stat-label { color: #6b7280; font-size: 0.7rem; }
.filters { background: #f9fafb; border-radius: 0.5rem; padding: 0.6rem; margin-bottom: 15px; display: flex; gap: 6px; flex-wrap: wrap; }
.filter-btn { padding: 0.3rem 0.8rem; border: 1px solid #e5e7eb; border-radius: 0.4rem; background: white; cursor: pointer; font-size: 0.75rem; }
...
.message-card { background: white; border-radius: 0.75rem; padding: 0.8rem; border: 1px solid #e5e7eb; transition: all 0.2s ease; }
.message-card:hover { transform: translateX(3px); border-color: #059669; }
.message-card.non_lu { background: #fef3c7; border-left: 3px solid #f59e0b; }
.message-header { display: flex; justify-content: space-between; flex-wrap: wrap; gap: 8px; margin-bottom: 6px; }
.message-expediteur { font-weight: 600; font-size: 0.85rem; }
.role-badge { background: #e5e7eb; padding: 1px 6px; border-radius: 15px; font-size: 0.6rem; margin-left: 5px; }
.message-sujet { font-weight: 600; color: #059669; font-size: 0.85rem; margin: 5px 0; }
.message-texte { color: #4b5563; font-size: 0.8rem; margin: 6px 0; line-height: 1.4; }
.message-date { font-size: 0.65rem; color: #9ca3af; }
.message-actions { display: flex; gap: 6px; margin-top: 6px; }
.btn-icon { background: none; border: none; padding: 0.2rem 0.5rem; cursor: pointer; border-radius: 0.3rem; font-size: 0.7rem; }
.btn-icon:hover { background: #f3f4f6; }
.btn-repondre { color: #059669; }
.btn-supprimer { color: #ef4444; }
.modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center; }
.modal.active { display: flex; }
.modal-content { background: white; border-radius: 0.75rem; padding: 1.2rem; max-width: 450px; width: 90%; }
.modal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.8rem; padding-bottom: 0.4rem; border-bottom: 1px solid #e5e7eb; }
...
.form-group input, .form-group select, .form-group textarea { width: 100%; padding: 0.4rem 0.6rem; border: 1px solid #e5e7eb; border-radius: 0.4rem; font-size: 0.8rem; }
.form-group textarea { min-height: 80px; }
.alert-success, .alert-error { padding: 0.5rem 0.8rem; border-radius: 0.4rem; margin-bottom: 0.8rem; font-size: 0.8rem; }
...
.info-box { background: #dbeafe; border-left: 3px solid #3b82f6; padding: 0.5rem 0.8rem; border-radius: 0.4rem; margin-bottom: 0.8rem; color: #1e40af; font-size: 0.75rem; display: flex; align-items: center; gap: 5px; }
.empty-state { text-align: center; padding: 40px 20px; background: #f9fafb; border-radius: 0.75rem; }
.empty-state div { font-size: 48px; }
.empty-state h3 { font-size: 1rem; margin-top: 8px; }
@media (max-width: 768px) { .messages-container { padding: 10px; } .page-header h1 { font-size: 1.2rem; } }
</style>

<div class="messages-container">
    <div class="page-header">
        <div>
            <h1>📨 Messagerie Admin ↔ Conseiller</h1>
            <p>Échangez avec les conseillers d'orientation</p>
        </div>
        <button class="btn-new-message" onclick="ouvrirModalNouveauMessage()">✨ Nouveau message</button>
    </div>

    <div class="info-box">💡 Vous pouvez envoyer et recevoir des messages uniquement avec les conseillers d'orientation.</div>
    ...
    <?php if (empty($messages)): ?>
        <div class="empty-state">
            <div>📭</div>
            <h3>Aucun message</h3>
            <button class="btn-new-message" onclick="ouvrirModalNouveauMessage()" style="margin-top:15px;">📝 Envoyer un message</button>
        </div>
    <?php else: ?>
